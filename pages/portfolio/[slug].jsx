...
const Portfolio = () => {
  ...
  return (
    <div
      className="h-auto min-h-[110vh] relative "
      style={{
    ...
    >
      ...
      <div className="font-Montserrat pt-[2vw]  mx-auto text-white flex items-center gap-[3vw] flex-wrap justify-evenly w-full md:w-[80%] lg:w-[60%]">
        {var_btns.map((each_btn) => {
          return (
            <Link href={`/portfolio/${each_btn.slug}`}>
              <button
                key={var_btns.indexOf(each_btn)}
                onClick={() => {
                  set_desc(each_btn.desc), display(each_btn.slug);
                }}
                className={` ${
                  each_btn.slug === new_path ? "bg-[orange]" : "bg-k_blue"
                }   mx-auto   duration-300 md:hover:text-[1.1vw] md:hover:font-thin text-center text-[2vw] h-[7vw] w-[20vw] md:max-w-[12vw] lg:text-[0.8vw] lg:max-w-[12vw]   md:h-[4vw] lg:h-[3vw]  text-center font-Monserrat   font-light   md:text-[1.1vw] font-light`}
              >
                {each_btn.title}
              </button>
            </Link>
      ...
      <div className="w-full flex items-center justify-evenly">
        <button className=" text-[2vw] font-Montserrat font-regular text-white text-center  bg-k_red  px-[4vw] py-[2vw] md:text-[0.7vw] md:px-[3em] md:py-[1em] hover:bg-[orange] hover:text-white hover:px-[3.2em] hover:py-[1.2em] hover:text-1.5xl duration-500 hover:font-bold ">
          {" "}
          HIRE US {">>"}{" "}
        </button>
      </div>
